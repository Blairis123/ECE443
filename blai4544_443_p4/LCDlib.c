...
void LCD_putc(char c) //Makes decisions about control char
{
    ...
    switch(c)
    {
        case '\r':
            ...
            {
                writeLCD(0, 0xC0); // Return to bottom left
            }
            break;
        case '\n':
            //Do control thing
...
// Clear the LCD of Crap
void LCD_clear(){
 
 // Don't clear when waiting to write
 busyLCD();
 
 // Reset using supplied data from LCD 341 documentation
 PMPSetAddress(0); //RS=0
 
 // Set position back to the top left
 writeLCD(0, 0x01);
}
