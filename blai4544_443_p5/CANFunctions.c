...
*                 Modified for ECE443 at the University of Idaho with Dr.J
*
************************************************************************/
 ...
 * Example:        CAN1RxMsgProcess();
  ***************************************************************************/
unsigned int CAN1RxMsgProcess( unsigned int *sensorData, unsigned int *speedData, unsigned char *PWMsetting)
{
  ...
  * what the PWM value should be set to*/
...
// MESSAGE PROCESSING!!!!!!!!!!
    *sensorData = (message->data[0] << 8) | (message->data[1]);
    *speedData = (message->data[2] << 24) | (message->data[3] << 16) | (message->data[4] << 8) | (message->data[5]);
    *PWMsetting = message->data[6];
 ...
 * isCAN2MsgReceived flag is updated by the CAN2 ISR. */
    ...
    #if ( configUSE_TRACE_FACILITY == 1 )
        vTracePrint(str, "CAN 2 Recvd");
    #endif   
    
	// Setup the variables needed
    unsigned char zeroByteFromMsg = 0;
    unsigned char RTRmsg = 0;
...
/* Check byte 0 of the data payload.*/
    RTRmsg = message->msgEID.RTR;
    if(RTRmsg ==0){ // Message is a data frame
        zeroByteFromMsg = message->data[0]; // Read the data
    }
    else{ // return a value that can't be a PWM setting so IO knows msg is RTR
        zeroByteFromMsg = 100;
    }
 ...
 * Example:  CAN2TxSendLEDMsg();
  ***************************************************************************/
void CAN1TxSendMsg(unsigned char PWMDutyCycle, short unsigned int RTRMsg)
{
...
//      message->msgEID.EID	= 0x4004;   // Send message to CAN2.
        ...
        // Do I want to send data or RTR?
        if(RTRMsg == 0){ // Message is data
            message->msgEID.RTR	= 0;        // Not an RTR message
            message->msgEID.DLC	= 1;        // Send one byte of data
            message->data[0] = PWMDutyCycle;// This is the data payload
        }
        else if(RTRMsg == 1){ // Message is an RTR
            message->msgEID.RTR	= 1;        // RTR message
            message->msgEID.DLC	= 0;        // Send one byte of data
        }
        else{ // There was a wrong input to the function!
            // Message isn't designated as an RTR or data message
            for( ;; );
        }
        
        //message->data[1] = firstByte;
 ...
 * Example:    CAN2TxSendLEDMsg();
  ***************************************************************************/
void CAN2TxSendMsg(unsigned int temperatureData, unsigned int speedData, unsigned char PWMSetting)
{
    // Determine how many bytes are needed to send message, should be 8 bytes
    //  for the max data length in a CAN message
    unsigned int msgSizeBytes = 7;
    
    // Array to save message bytes to
    unsigned char msgDataByteArry[msgSizeBytes];
    
    // ASSIGN DATA VALUES TO ARRAY TO ITTERATE OVER-----------------------------
        // Temperature data (Total data type is unsigned short int - 2 Bytes)
    // And with 0th byte, shift 12 bits right & cast as an unsigned char
    msgDataByteArry[0] = (unsigned char) ( (temperatureData & 0xFF00 ) >> 8);
    // And with 1st byte, shift 4 bits right & cast as an unsigned char
    msgDataByteArry[1] = (unsigned char) (  temperatureData & 0x00FF );

        // Speed data
    // And with 0th byte, shift 12 bits right & cast as an unsigned char
    msgDataByteArry[2] = (unsigned char) ( (speedData & 0xFF000000) >> 24);
    // And with 1st byte, shift 8 bits right & cast as an unsigned char
    msgDataByteArry[3] = (unsigned char) ( (speedData & 0x00FF0000) >> 16);
    // And with 2nd byte, shift 4 bits right & cast as an unsigned char
    msgDataByteArry[4] = (unsigned char) ( (speedData & 0x0000FF00) >> 8);
    // And with 3rd byte & cast as an unsigned char
    msgDataByteArry[5] = (unsigned char) (  speedData & 0x000000FF);
    
        // Current PWM setting data byte
    msgDataByteArry[6] = PWMSetting;
    
/* This function will send a message to CAN1 with EID 8004001. The data payload
 * size is whatever it takes to send the temperature data speed data and the
 * current PWM setting */
...
/* Form a Extended ID message. Start by clearing the buffer. */
		...
		message->msgEID.IDE = 1;            /* IDE = 1 means EID message. */
        message->msgEID.RTR = 0;            /* Not an RTR message. */
        message->msgEID.DLC = msgSizeBytes; /* Send msgSizeBytes bytes of data */
        
        // Loop through and assign data array to message bytes
        unsigned int i = 0;
        while(i < msgSizeBytes){
            message->data[i] = msgDataByteArry[i];
            i++;
        }
